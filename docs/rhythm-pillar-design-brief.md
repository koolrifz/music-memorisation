...
**Confirmed:** the same merge-into-one-bracket rule applies *across full beat numbers*, not just within a beat's own subdivisions — a dotted half spanning beats 2–3 renders `1 (2 3)` (one open bracket, one close), never `1 (2) (3)`. Rob's own framing: "brackets used sparingly... if there are more than one syllable that needs to bracket, then they are bracketed in one open and one close." One bracket pair per contiguous non-Play run, at any span, sub-beat or cross-beat — no special-casing by span length.

**Also confirmed — Hold and Rest sharing one bracket symbol is deliberate, not an oversight to fix later.** Rob's own precedent: ties and slurs are the same curved line in standard notation, and nobody confuses them because the context around the line makes it unambiguous — a tie connects two noteheads at the same pitch, a slur connects different ones. Same logic here: Hold's bracket sits under a note that's still sounding, Rest's bracket sits under an actual rest glyph on the staff above it, so which one it is is always legible from context, never from the bracket itself needing a second visual treatment. This closes out §5 item 11's original amber/blue distinction for good — that was solving a problem this design doesn't have.
...
1. ~~Does bracket-merging span full beat numbers~~ — **resolved:** yes, one open/one close bracket per contiguous non-Play run at any span. See §9.3.
